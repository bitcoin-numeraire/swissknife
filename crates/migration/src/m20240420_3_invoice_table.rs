...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        if manager.get_database_backend() == DatabaseBackend::Sqlite {
            table.col(uuid_null(Invoice::BtcOutputId)).foreign_key(
                ForeignKey::create()
                    .name("fk_invoice_btc_output")
                    .from(Invoice::Table, Invoice::BtcOutputId)
                    .to(BtcOutput::Table, BtcOutput::Id)
...
pub(crate) enum Invoice {
    ...
    UpdatedAt,
    ExpiresAt,
    // Bitcoin L1 support (added in m20251224_162546)
    BtcOutputId,
}
