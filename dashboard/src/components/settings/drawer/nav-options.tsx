    ...
    <div>
      ...
      <Box gap={1.5} display="flex" sx={{ mt: 1.5 }}>
        {options.layouts.map((option) => (
          <LayoutOption key={option} option={option} selected={value.layout === option} onClick={() => onClickOption.layout(option)} />
        ))}
      </Box>
    </div>
  );
    ...
    <div>
      ...
      <Box gap={1.5} display="flex" sx={{ mt: 1.5 }}>
        {options.colors.map((option) => (
          <ColorOption key={option} option={option} selected={value.color === option} onClick={() => onClickOption.color(option)} />
        ))}
      </Box>
    </div>
  );
...
export function ColorOption({ option, selected, sx, ...other }: OptionProps) {
  return (
    <ButtonBase
      ...
      {...other}
    >
      <SvgColor src={`${CONFIG.site.basePath}/assets/icons/setting/ic-sidebar-${option === 'integrate' ? 'outline' : 'filled'}.svg`} />
