...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // wallet_balance becomes the authoritative read source in this release. It was first
        // backfilled in m20260609_143601, but incoming credits were not materialized until now,
        // so recompute every row from the ledger to remove drift before reads switch over.
        ...
        if negative_count > 0 {
            return Err(DbErr::Migration(
                "wallet balance recompute produced negative balances; manual reconciliation required".to_string(),
            ));
        }
    ...
    async fn down(&self, _manager: &SchemaManager) -> Result<(), DbErr> {
        // Balances are recomputed from the ledger; there is no prior snapshot to restore.
        Ok(())
    }
}
