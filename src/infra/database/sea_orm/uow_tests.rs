...
mod local_credential_migration {
    use super::*;
    use crate::domains::{
        account::{LocalCredentialRepository, LOCAL_AUTH_INITIALIZED_KEY},
        system::ConfigRepository,
    };
    use crate::infra::database::sea_orm::{SeaOrmConfigRepository, SeaOrmLocalCredentialRepository};

    async fn legacy_database() -> DatabaseConnection {
        let conn = connect_unmigrated().await;
        let preceding = Migrator::migrations()
            .iter()
            .position(|migration| migration.name() == "m20260904_192903_local_jwt_credentials")
            .unwrap();
        Migrator::up(&conn, Some(preceding as u32)).await.unwrap();
        conn
    }

    #[tokio::test]
    async fn moves_the_legacy_hash_without_replacing_account_identity_or_wallets() {
        let conn = legacy_database().await;
        let account_repository = SeaOrmAccountRepository::new(conn.clone());
        ...
        let asset = SeaOrmAssetRepository::new(conn.clone())
            ...
            .unwrap()
            .unwrap();
        let wallet_repository = SeaOrmWalletRepository::new(conn.clone());
        let wallet = wallet_repository.upsert(account.id, asset.id).await.unwrap();
        wallet_repository.credit(wallet.id, 123_000).await.unwrap();
        let config_repository = SeaOrmConfigRepository::new(conn.clone());
        let legacy_hash = bcrypt::hash("legacy-owner-password", 4).unwrap();
        config_repository
            .insert("password_hash", serde_json::json!(legacy_hash))
            .await
            .unwrap();

        Migrator::up(&conn, None).await.unwrap();
        let credential = SeaOrmLocalCredentialRepository::new(conn.clone())
            .find(account.id)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(credential.identity_id, account.identity.unwrap().id);
        assert_eq!(credential.password_hash.as_deref(), Some(legacy_hash.as_str()));
        assert!(credential.enabled);
        assert!(config_repository.find("password_hash").await.unwrap().is_none());
        assert_eq!(
            config_repository.find(LOCAL_AUTH_INITIALIZED_KEY).await.unwrap(),
            Some(serde_json::json!(true))
        );
        let after = account_repository.find(account.id).await.unwrap().unwrap();
        assert_eq!(after.display_name.as_deref(), Some("Existing owner"));
        assert_eq!(after.permissions, Some(vec![Permission::WriteAccount]));
        assert_eq!(after.wallets[0].id, wallet.id);
        assert_eq!(balance(&conn, wallet.id).await, (123_000, 0));
    }

    #[tokio::test]
    async fn a_missing_legacy_identity_rolls_back_the_entire_migration() {
        let conn = legacy_database().await;
        let config_repository = SeaOrmConfigRepository::new(conn.clone());
        config_repository
            .insert(
                "password_hash",
                serde_json::json!(bcrypt::hash("legacy-owner-password", 4).unwrap()),
            )
            .await
            .unwrap();
        assert!(Migrator::up(&conn, None).await.is_err());
        assert!(!migration::SchemaManager::new(&conn)
            .has_table("local_credential")
            .await
            .unwrap());
        assert!(config_repository.find("password_hash").await.unwrap().is_some());
        assert!(config_repository
            .find(LOCAL_AUTH_INITIALIZED_KEY)
            .await
            .unwrap()
            .is_none());
    }
